...
class DessMonitorAPI:
    ...
    def _is_token_expired(self) -> bool:
        ...
        _LOGGER.debug("Token expiration check: current=%d, expires=%d, expired=%s", 
                     current_time, self.token_expire, expired)
        return expired

    async def _make_request(self, action: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        """Make API request."""
        ...
        _LOGGER.debug("Making %s request with %d parameters", action, len(params) if params else 0)
            ...
            async with async_timeout.timeout(30):
                async with self._session.get(url) as response:
                    ...
                    if data.get("err", 0) != 0:
                        ...
                        _LOGGER.error("API returned error %s for action '%s': %s", error_code, action, error_msg)
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            _LOGGER.debug("Authentication parameters: %s", {k: v if k != 'usr' else '***' for k, v in auth_params.items()})
            ...
            if "dat" in response:
                ...
                _LOGGER.debug("Authentication response data keys: %s", list(data.keys()))
                ...
                if expire_duration:
                    ...
                    _LOGGER.debug("Token will expire at timestamp: %d (in %d seconds)", self.token_expire, expire_duration)
                else:
                    self.token_expire = None
                    _LOGGER.warning("No expiration duration provided by API")
                
                _LOGGER.info("Successfully authenticated with DessMonitor API, token valid for %d seconds, expires at: %s", 
                           expire_duration or 0, self.token_expire)
                return True
                ...
                for project in projects:
                    ...
                    if pid:
                        try:
                            ...
                            while True:
                                collectors_response = await self._make_request("webQueryCollectorsEs", {
                                    "pid": pid,
                                    "page": page,
                                    "pagesize": pagesize
                                })
                                ...
                                if "dat" in collectors_response:
                                    ...
                                    if project_collectors:
                                        ...
                                        _LOGGER.debug("Found %d collectors in project %s (page %d), total so far: %d/%d", 
                                                    current_page_size, pid, page, total_collectors, total_from_api)
                                        
                                        # Check if we've got all collectors
                                        if total_collectors >= total_from_api or current_page_size < pagesize:
                                            break
                            ...
                            _LOGGER.info("Retrieved %d total collectors for project %s", total_collectors, pid)
                                    
                        except Exception as err:
                            _LOGGER.warning("Failed to get collectors for project %s: %s", pid, err)
            ...
            if not collectors:
                ...
                try:
                    ...
                    if "dat" in direct_response:
                        ...
                        _LOGGER.debug("Direct collector query response: %s", list(direct_response["dat"].keys()) if isinstance(direct_response["dat"], dict) else "non-dict response")
    ...
    async def get_collector_devices(self, pn: str) -> dict[str, Any]:
        ...
        if device_count > 0:
            for i, device in enumerate(devices_data.get("dev", [])):
                _LOGGER.debug("Device %d: SN=%s, devcode=%s, devaddr=%s", 
                            i, device.get("sn"), device.get("devcode"), device.get("devaddr"))
    ...
    async def get_device_last_data(
        ...
        _LOGGER.debug("Fetching device data: pn=%s, devcode=%s, devaddr=%s, sn=%s", 
