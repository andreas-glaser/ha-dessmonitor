...
from homeassistant.const import UnitOfElectricCurrent, UnitOfElectricPotential, UnitOfFrequency, UnitOfPower, UnitOfTemperature
...
async def async_setup_entry(
    ...
    _LOGGER.debug("Setting up DessMonitor sensors for config entry: %s", config_entry.entry_id)
    coordinator: DessMonitorDataUpdateCoordinator = hass.data[DOMAIN][config_entry.entry_id]
    ...
    if coordinator.data:
        ...
        for device_sn, device_info in coordinator.data.items():
            ...
            _LOGGER.debug("Processing device %s with %d data points", device_sn, len(device_data))
            ...
            for data_point in device_data:
                ...
                if sensor_type in SENSOR_TYPES:
                    ...
                    if sensor_key not in seen_sensors:
                        ...
                        supported_sensors += 1
                        _LOGGER.debug("Created sensor: %s for device %s", sensor_type, device_sn)
                    else:
                        duplicate_sensors += 1
                        _LOGGER.warning("Duplicate sensor detected: %s for device %s", sensor_type, device_sn)
                else:
                    _LOGGER.debug("Unsupported sensor type: %s for device %s", sensor_type, device_sn)
            
            _LOGGER.info("Device %s: created %d sensors, skipped %d duplicates", 
                        device_sn, supported_sensors, duplicate_sensors)
    else:
...
class DessMonitorSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(
        ...
        _LOGGER.debug("Initialized sensor: name='%s', unique_id='%s', device='%s', type='%s'", 
                     self._attr_name, self._attr_unique_id, device_sn, sensor_type)
    ...
    def device_info(self) -> DeviceInfo:
        ...
        _LOGGER.debug("Device info for %s: name='%s', pn='%s', firmware='%s'", 
                     self._device_sn, device_name, collector_pn, firmware)
    ...
    def native_value(self) -> str | float | None:
        ...
        if not device_info:
            _LOGGER.debug("No device info found for device %s (sensor: %s)", self._device_sn, self._attr_unique_id)
            return None
        ...
        for data_point in device_data:
            if data_point.get("title") == self._sensor_type:
                ...
                _LOGGER.debug("Raw value for sensor %s (%s): %s (type: %s)", 
                            self._attr_unique_id, self._sensor_type, value, type(value).__name__)
                
                try:
                    if value is not None and value != "":
                        numeric_value = float(value)
                        _LOGGER.debug("Converted value for sensor %s: %s -> %s", 
                                    self._attr_unique_id, value, numeric_value)
                        return numeric_value
                    return value
                except (ValueError, TypeError) as e:
                    _LOGGER.debug("Could not convert value to float for sensor %s: %s (%s)", 
                                self._attr_unique_id, value, e)
                    return str(value) if value is not None else None
        
        _LOGGER.debug("No matching data point found for sensor %s (looking for: %s)", 
                     self._attr_unique_id, self._sensor_type)
        return None
