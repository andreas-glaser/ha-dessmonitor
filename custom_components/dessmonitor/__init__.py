        ...
        try:
            ...
            if collectors:
                ...
                try:
                    ...
                    projects_response = await self.api._make_request("queryPlants", {"pagesize": 1})
                        ...
                        if projects:
                            ...
                            for device_sn, device_info in data.items():
                                if device_sn in summary_data:
                                    ...
                                    summary_types = [p.get("title") for p in summary_points]
                                    _LOGGER.debug(
            ...
            if update_count % 50 == 1:  # Update on first call and every 50th update (less frequent)
                            ...
                            if field_data.get("id"):
                                ...
                                try:
                                    ...
                                    if value_response.get("err") == 0:
                                        current_value = value_response.get("dat", {}).get("value")
                                        _LOGGER.debug(
                                            "Got control field value for %s (%s): %s",
                                            field_name, control_id, current_value
                                        )
                                    else:
                                        _LOGGER.debug(
                                            "No value available for control field %s (%s): %s",
                                            field_name, control_id, value_response.get("desc", "Unknown error")
                                        )
                                except Exception as err:
                                    _LOGGER.debug(
...
async def async_get_device_diagnostics(
    ...
    for data_point in device_data:
        ...
        if any(keyword in title.lower() for keyword in [
            "setting", "priority", "voltage setting", "cutoff", "capacity",
            "type", "configuration"
            ...
            "status", "operating mode", "state", "online", "connectivity"
        ]):
    ...
    try:
        ...
        for field_name, field_data in control_fields.items():
            if field_data["type"] == "options":
                ...
                sensor_value = next(
                    (dp.get("val") for dp in device_data if dp.get("title") == field_name),
                    None
                )
                ...
                if sensor_value and field_name in priority_mapping:
                    for option_text, option_key in priority_mapping[field_name].items():
                        if str(sensor_value).upper() == option_text.upper():
                            current_value = next(
                                (opt["val"] for opt in field_data["options"] if opt["key"] == str(option_key)),
                                option_text
                            )
                            break
                elif sensor_value:
                ...
                if field_name == "Output Voltage" and current_value == "Not Set":
                    output_voltage = next(
                        (dp.get("val") for dp in device_data if dp.get("title") == "Output Voltage"),
                        None
                    )
                    if output_voltage:
                        voltage_val = str(float(output_voltage))
                        current_value = next(
                            (opt["val"] for opt in field_data["options"] if opt["key"] == voltage_val),
                ...
                diagnostics["configuration"][field_name] = {
                    "value": current_value,
                    "unit": "",
                    "options": [f"{opt['key']}: {opt['val']}" for opt in field_data["options"]],
                    "id": field_data["id"]
                }
            else:
                current_value = next(
                ...
                voltage_fields = [
                    "High DC Protection Voltage", "Bulk Charging Voltage", 
                    "Floating Charging Voltage", "Low DC Protection Voltage In Mains Mode",
                    "Low DC Protection Voltage In Off-Grid Mode"
                ]
                
                if any(vf in field_name for vf in voltage_fields) and current_value == "Not Set":
                ...
                diagnostics["configuration"][field_name] = {
                    "value": current_value,
                    "unit": "V" if "Voltage" in field_name else ("A" if "Current" in field_name else ""),
                    "id": field_data["id"]
                }
        ...
        _LOGGER.debug(
            ...
            len(control_fields), len(device_parameters), device_sn
        )
        
    except Exception as err:
        _LOGGER.warning("Failed to fetch control fields/parameters for device %s: %s", device_sn, err)
        diagnostics["battery_config_error"] = str(err)
    
    diagnostics["summary"] = {
        "total_sensors_created": len([dp for dp in device_data if dp.get("title") in coordinator.data.get("sensor_types", [])]),
        "last_update": next((dp.get("val") for dp in device_data if dp.get("title") == "Timestamp"), "Unknown"),
    ...
    _LOGGER.debug("Generated diagnostics for device %s with %d config items", device_sn, len(diagnostics["configuration"]))
