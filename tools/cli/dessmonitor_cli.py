...
Usage:
    ...
    python dessmonitor_cli.py auth --username USER
...
def _load_constants():
    """Read the shared platform definitions without requiring Home Assistant."""
    path = (
        Path(__file__).resolve().parents[2] / "custom_components/dessmonitor/const.py"
    )
    spec = importlib.util.spec_from_file_location("dessmonitor_cli_constants", path)
    if spec is None or spec.loader is None:
        raise RuntimeError("Unable to load DessMonitor platform definitions")
...
class DessMonitorCLI:
    ...
    def __init__(self, api_profile: str | None = None):
        self._requested_api_profile = api_profile
        self._set_api_profile(
            api_profile if api_profile is not None else _CONSTANTS.DEFAULT_API_PROFILE
        )
    ...
    def _set_api_profile(self, profile: str) -> None:
        self.api_profile = _CONSTANTS.resolve_api_profile({"api_profile": profile})
        self._profile = _CONSTANTS.API_PROFILES[self.api_profile]
    ...
    async def __aenter__(self):
        ...
        self.session = aiohttp.ClientSession(
            connector=connector,
            timeout=aiohttp.ClientTimeout(total=_CONSTANTS.API_REQUEST_TIMEOUT_SECONDS),
        )
        return self
    ...
    def _create_signature(self, action_string: str, salt: str) -> str:
        """Sign the exact encoded query appended to the request URL."""
    ...
    async def _make_request(
        ...
        if action != self._profile["auth_action"] and self._is_token_expired():
            if not await self.authenticate_from_config():
                raise RuntimeError("Authentication failed; run the auth command again")

        if "source" in params:
            params = {**params, "source": self._profile["source"]}

        # Generate salt and signature
        salt = str(int(time.time() * 1000))
        action_string = f"&action={action}" + "".join(
            f"&{key}={quote_plus(str(value))}" for key, value in params.items()
        )
        signature = self._create_signature(action_string, salt)

        # Build URL
        url = f"{self.base_url}?sign={signature}&salt={salt}"
        if self.token and action != self._profile["auth_action"]:
        ...
        try:
            async with self.session.get(URL(url, encoded=True)) as response:
            ...
            raise RuntimeError(
                f"API transport failed for action {action} " f"({type(err).__name__})"
            ) from None
    ...
    async def authenticate(
        self, username: str, password: str, company_key: str
    ) -> bool:
        ...
        auth_params = {
            ...
            "_app_client_": self._profile["app_client"],
            "_app_id_": self._profile["app_id"],
            "_app_version_": _CONSTANTS.VERSION,
        }

        try:
            response = await self._make_request(
                self._profile["auth_action"], auth_params
            )
            logger.debug(
            ...
            logger.info("Authenticated with %s; credentials saved", self.api_profile)
            return True
    ...
    async def authenticate_from_config(self) -> bool:
        ...
        saved_profile = _CONSTANTS.resolve_api_profile(config)
        if (
            self._requested_api_profile is not None
            and saved_profile != self.api_profile
        ):
            raise ValueError(
                "Saved credentials belong to another platform; run auth with --api-profile first"
            )
        self._set_api_profile(saved_profile)
        ...
        if (
            ...
            and config.get("token_expires", 0) > time.time()
        ):
        ...
        return await self.authenticate(self.username, self.password, self.company_key)
    ...
    async def get_collectors(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            await self._make_request("queryCollectorCountEs")
    ...
    async def get_devices(self, collector_pn: str) -> Dict[str, Any]:
        ...
        response = await self._make_request(
            "queryCollectorDevices", {"pn": collector_pn}
        )
    ...
    async def _find_device_info(self, device_sn: str) -> Optional[Dict[str, Any]]:
        ...
        for collector in collectors:
            ...
            try:
                ...
                devices = devices_data.get(
                    "dev", []
                )  # Note: key is "dev", not "devices"
            ...
            try:
                ...
                if device_data:
                    return [
                        {
                            "title": "Device Info",
                            "devcode": device_data.get("devcode"),
                            **device_data,
                        }
                    ]
            except Exception as err:
                logger.debug("Device info fallback failed (%s)", type(err).__name__)
    ...
    async def get_device_parameters(
        ...
        logger.debug("Collected %d parameters for the selected device", len(formatted))
        return formatted
    ...
    async def get_sp_key_parameters(self, device_sn: str) -> Dict[str, Any]:
        ...
        raise Exception(
            f"Unable to query SP key parameters for {device_sn}: {last_err}"
        )

    async def set_device_control_value(
        self, device_sn: str, param_id: str, value: str
    ) -> Dict[str, Any]:
...
"""
    ...
    async def analyze_device_for_devcode(self, device_sn: str) -> Dict[str, Any]:
        ...
        collector_alias = (
            device_lookup.get("collector_alias") if device_lookup else "Unknown"
        )
        ...
        devcode = (
            device_info.get("devcode")
            or collector_info.get("devcode")
            or devcode_from_lookup
        )

        # Analyze sensor types and detect patterns
        operating_modes: dict[str, int] = {}
        output_priorities: dict[str, int] = {}
        charger_priorities: dict[str, int] = {}
        sensor_titles: list[dict[str, Any]] = []
        unique_sensors: set[str] = set()
        potential_typos: list[dict[str, str]] = []
        ...
        for point in sensor_data:
            ...
            unique_sensors.add(title)
            sensor_titles.append(
                {
                    "title": title,
                    "value": value,
                    "unit": point.get("unit", ""),
                ...
                if typo in title_lower and correct not in title_lower:
                    ...
                    if typo.capitalize() in title:
                        suggested = suggested.replace(
                            typo.capitalize(), correct.capitalize()
                        )
                    # Replace uppercase version
                    if typo.upper() in title:
                        suggested = suggested.replace(typo.upper(), correct.upper())

                    potential_typos.append({"original": title, "suggested": suggested})

            # Detect units in values (W, V, A, Hz, °C, %, kWh, etc.)
            value_str = str(value)
            if any(
                unit in value_str
                for unit in ["W", "V", "A", "Hz", "°C", "°F", "%", "kWh", "Wh"]
            ):
                if title not in unit_patterns:
                    unit_patterns[title] = []
                unit_patterns[title].append(value_str)

            # Check for operating modes
            if "operating" in title_lower and "mode" in title_lower:
                mode_val = str(value)
                operating_modes[mode_val] = operating_modes.get(mode_val, 0) + 1

            # Check for priorities
            if "priority" in title_lower:
                priority_val = str(value)
                if "output" in title_lower:
                    output_priorities[priority_val] = (
                        output_priorities.get(priority_val, 0) + 1
                    )
                elif "charg" in title_lower:
                    charger_priorities[priority_val] = (
                        charger_priorities.get(priority_val, 0) + 1
                    )
        ...
        analysis_result = {
            ...
            "collector_identity": (
                "sha256:"
                + hashlib.sha256(
                    str(device_lookup.get("pn", "")).encode("utf-8")
                ).hexdigest()[:12]
            ...
            "operating_modes": sorted(operating_modes.keys()),
            "output_priorities": sorted(output_priorities.keys()),
            "charger_priorities": sorted(charger_priorities.keys()),
            "sensor_titles": sorted(unique_sensors),
            "potential_typos": potential_typos,
            "unit_patterns": {
                k: list(set(v))[:3] for k, v in unit_patterns.items()
            },  # Show sample units
            "sample_data": sensor_titles[:20],  # First 20 samples for better context
        ...
        if potential_typos:
            logger.info(f"Found {len(potential_typos)} potential typos in sensor names")
...
def setup_argparser() -> argparse.ArgumentParser:
    ...
    auth_parser = subparsers.add_parser(
        "auth", help="Authenticate with DessMonitor API"
    )
    ...
    auth_parser.add_argument(
        "--company-key",
        default=_CONSTANTS.DEFAULT_COMPANY_KEY,
        help="Company key (keep the default unless supplied by your installer)",
    )
    auth_parser.add_argument(
        "--api-profile",
        choices=list(_CONSTANTS.API_PROFILES),
        default=_CONSTANTS.DEFAULT_API_PROFILE,
        help="Account platform: dessmonitor_ess = DessMonitor / SmartESS (default); "
        "shinemonitor_solar = SmartClient for Solar / ShineMonitor",
    )

    # Collectors command
    collectors_parser = subparsers.add_parser(
        "collectors", help="List all data collectors"
    )
    ...
    devices_parser = subparsers.add_parser(
        "devices", help="List devices for a collector"
    )
    devices_parser.add_argument(
        "--pn", required=True, help="Collector part number (PN)"
    )
    devices_parser.add_argument(
        "--raw", action="store_true", help="Print raw JSON response"
    )
    ...
    sp_parser = subparsers.add_parser(
        "sp-keys", help="Query SP key parameters for a device"
    )
    ...
    set_config_parser = subparsers.add_parser(
        "set-config", help="Set a device configuration value"
    )
    set_config_parser.add_argument(
        "--device-sn", required=True, help="Device serial number"
    )
    set_config_parser.add_argument(
        "--param-id", required=True, help="Parameter ID (from analyze command)"
    )
    set_config_parser.add_argument("--value", required=True, help="New value to set")

    # Analyze command
    analyze_parser = subparsers.add_parser(
        "analyze", help="Analyze device for devcode mapping"
    )
    ...
    analyze_parser.add_argument(
        "--template", action="store_true", help="Generate devcode template Python file"
    )
    analyze_parser.add_argument(
        "--local-report",
    ...
    verify_parser = subparsers.add_parser(
        "verify", help="Verify integrity of an analysis JSON file"
    )
...
async def main():
    ...
    async with DessMonitorCLI(api_profile=getattr(args, "api_profile", None)) as cli:
        try:
            if args.command == "auth":
                password = args.password or getpass.getpass("Account password: ")
                success = await cli.authenticate(
                    args.username, password, args.company_key
                )
                if not success:
                    sys.exit(1)
                ...
                if args.raw:
                    ...
                    for collector in collectors:
                        ...
                        print(
                            f"PN: {pn} | Alias: {alias} | Project: {project} | Status: {status}"
                        )
                ...
                if args.raw:
                    ...
                    for device in devices:
                        ...
                        print(
                            f"SN: {sn} | DevCode: {devcode} | DevAddr: {devaddr} | Alias: {alias}"
                        )
                    ...
                    if isinstance(dat, list):
                        ...
                        if isinstance(keys_list, list):
                            ...
                            for k, v in list(dat.items())[:100]:
                                print(
                                    f"- {k}: {v if isinstance(v, (str, int, float)) else type(v).__name__}"
                                )
                    else:
                        print(dat)

            elif args.command == "set-config":
                resp = await cli.set_device_control_value(
                    args.device_sn, args.param_id, args.value
                )
                print(json.dumps(resp, indent=2))
                if resp.get("err") == 0:
                    print(
