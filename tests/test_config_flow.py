...
async def test_hybrid_validation_preserves_entered_collector_list(
    ...
    assert defaults[CONF_LOCAL_COLLECTOR_IPS] == ("192.168.10.50, 192.168.10.51")
...
@pytest.mark.usefixtures("mock_validate_input", "mock_setup_entry")
async def test_hybrid_flow_preserves_solar_profile(hass):
    result = await hass.config_entries.flow.async_init(
        DOMAIN, context={"source": config_entries.SOURCE_USER}
    )
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"], {"next_step_id": "hybrid"}
    )
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {
            ...
            "api_profile": "shinemonitor_solar",
        },
    )
    result = await hass.config_entries.flow.async_configure(
        result["flow_id"],
        {
            "local_listen_ip": "192.168.1.10",
            "local_collector_ips": "192.168.1.20",
            "local_poll_interval": "5",
            "local_redirect_confirmed": True,
        },
    )
    assert result["type"] is FlowResultType.CREATE_ENTRY
    assert result["data"]["api_profile"] == "shinemonitor_solar"
    assert (
        hass.config_entries.async_entries(DOMAIN)[0].unique_id
        == "shinemonitor_solar:test-user"
    )


@pytest.mark.parametrize("step", [CONNECTION_TYPE_CLOUD, CONNECTION_TYPE_HYBRID])
async def test_platform_selector_defaults_to_existing_service(hass, step):
    from custom_components.dessmonitor.const import (
        CONF_API_PROFILE,
        DEFAULT_API_PROFILE,
    )
    ...
    assert _form_default(result, CONF_API_PROFILE) == DEFAULT_API_PROFILE
    field = _serialized_form_field(result, CONF_API_PROFILE)
    options = field["selector"]["select"]["options"]
    assert options == [
        {"value": "dessmonitor_ess", "label": "DessMonitor / SmartESS (default)"},
        {
            "value": "shinemonitor_solar",
            "label": "SmartClient for Solar / ShineMonitor",
        },
    ]


@pytest.mark.usefixtures("mock_validate_input", "mock_setup_entry")
async def test_existing_cloud_account_is_not_duplicated(hass):
    entry = MockConfigEntry(
        domain=DOMAIN, unique_id=BASE_INPUT[CONF_USERNAME], data=BASE_INPUT
    )
    entry.add_to_hass(hass)
    result = await _start_cloud_flow(hass)
    result = await hass.config_entries.flow.async_configure(
